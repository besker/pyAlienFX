...
class pyAlienFX_GUI():
	...
	def main(self):
		...
		if not self.Advanced_Mode:
			self.AlienFX_Inside_vBox.get_children()[2].hide()
	...
	def AlienFX_Main(self):
		...
		#Background Colors !
		bg_rgba = Gdk.RGBA()
		bg_rgba.parse(self.background_color)
		fg_rgba = Gdk.RGBA()
		fg_rgba.parse(self.text_color)

		self.AlienFX_ComputerName_EventBox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		self.AlienFX_ComputerName_Label.override_color(gtk.StateFlags.NORMAL, fg_rgba)
		self.AlienFX_Configurator_ScrollWindow.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		self.AlienFX_Profiles_Eventbox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		self.AlienFX_Profile_Chooser_Eventbox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		self.AlienFX_Tempo_EventBox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		#Optional to be deleted eventually !
		#====----====
		self.AlienFX_Color_Eventbox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		self.AlienFX_Computer_Eventbox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		self.AlienFX_Main_Eventbox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		self.AlienFX_Configurator_Eventbox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
		#====----====

		#========Creation of different GTK elements not specified in Glade=========
		self.AlienFX_Tempo_VBox = gtk.Box(orientation=gtk.Orientation.VERTICAL)
		self.AlienFX_Tempo_Label = gtk.Label()
		self.AlienFX_Tempo_Label.set_label(" Tempo ")
		self.AlienFX_Tempo_Label.override_color(gtk.StateFlags.NORMAL, fg_rgba)
		ajustement = gtk.Adjustment(value=(self.configuration.speed // 256), lower=1, upper=255, step_increment=1, page_increment=16)
		self.AlienFX_Tempo_Ruler = gtk.Scale(orientation=gtk.Orientation.VERTICAL, adjustment=ajustement)
		...
		self.AlienFX_Tempo_VBox.pack_start(self.AlienFX_Tempo_Label, False, False, 0)
		self.AlienFX_Tempo_VBox.pack_start(self.AlienFX_Tempo_Ruler, False, False, 0)
		Apply = gtk.Button()
	...
	def Updtate_Profile_Combobox(self):
		try:
			...
			pass
		self.AlienFX_Profiles_Combobox = gtk.ComboBoxText()
	...
	def Create_Border(self, Type, Inside, Label=None, zone=None, confId=None):
		...
		if Type == 0:
			...
			TopBox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			UL = gtk.EventBox()
			ULi = gtk.Image()
			ULi.set_from_file(self.Image_DB.AlienFX_Cadre_0_Up_Left)
			UL.add(ULi)
			UL.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			UL2 = gtk.EventBox()
			UL2i = gtk.Image()
			UL2i.set_from_file(self.Image_DB.AlienFX_Cadre_0_Up_Left2)
			UL2.add(UL2i)
			UL2.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			if len(Label) < 11:
				Title_width = (11) * 9
			else:
				Title_width = (len(Label) * 9)
			UMbg = GdkPixbuf.Pixbuf.new_from_file_at_scale(self.Image_DB.AlienFX_Cadre_0_Up_Middle, width=Title_width, height=31, preserve_aspect_ratio=False)
			UM = gtk.EventBox()
			UM.set_size_request(width=Title_width, height=-1)
			UM.connect('draw', self.__textbackground, UMbg)
			text = gtk.Label('<span size="11000" color="#00FFFF">%s</span>' % Label)
			text.set_use_markup(True)
			UM.add(text)
			UM.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			UR = gtk.EventBox()
			URi = gtk.Image()
			URi.set_from_file(self.Image_DB.AlienFX_Cadre_0_Up_Right)
			UR.add(URi)
			UR.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			UR2 = gtk.EventBox()
			UR2i = gtk.Image()
			UR2i.set_from_file(self.Image_DB.AlienFX_Cadre_0_Up_Right2)
			UR2.add(UR2i)
			UR2.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			L = gtk.EventBox()
			Li = gtk.Image()
			Li.set_from_file(self.Image_DB.AlienFX_Cadre_0_Left)
			L.add(Li)
			L.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			R = gtk.EventBox()
			Ri = gtk.Image()
			Ri.set_from_file(self.Image_DB.AlienFX_Cadre_0_Right)
			R.add(Ri)
			R.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			BL = gtk.EventBox()
			BLi = gtk.Image()
			BLi.set_from_file(self.Image_DB.AlienFX_Cadre_0_Bottom_Left)
			BL.add(BLi)
			BL.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			BL2 = gtk.EventBox()
			BL2i = gtk.Image()
			BL2i.set_from_file(self.Image_DB.AlienFX_Cadre_0_Bottom_Middle_Left)
			BL2.add(BL2i)
			BL2.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			BM = gtk.EventBox()
			BMi = gtk.Image()
			BMbg = GdkPixbuf.Pixbuf.new_from_file_at_scale(self.Image_DB.AlienFX_Cadre_0_Bottom_Middle, width=((81 + Title_width) - 165), height=16, preserve_aspect_ratio=False)
			BMi.set_from_pixbuf(BMbg)
			BMi.set_size_request(width=((81 + Title_width) - 165), height=-1)
			BM.add(BMi)
			BM.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			BR2 = gtk.EventBox()
			BR2i = gtk.Image()
			BR2i.set_from_file(self.Image_DB.AlienFX_Cadre_0_Bottom_Middle_Right)
			BR2.add(BR2i)
			BR2.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			BR = gtk.EventBox()
			BRi = gtk.Image()
			BRi.set_from_file(self.Image_DB.AlienFX_Cadre_0_Bottom_Right)
			BR.add(BRi)
			BR.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			Inside.set_size_request(width=(81 + Title_width) - 33, height=-1)
			TopBox.pack_start(UL, False, False, 0)
			TopBox.pack_start(UL2, False, False, 0)
			TopBox.pack_start(UM, False, False, 0)
			TopBox.pack_start(UR2, False, False, 0)
			TopBox.pack_start(UR, False, False, 0)
			MiddleBox.pack_start(L, False, False, 0)
			MiddleBox.pack_start(Inside, False, False, 0)
			MiddleBox.pack_start(R, False, False, 0)
			BottomBox.pack_start(BL, False, False, 0)
			BottomBox.pack_start(BL2, False, False, 0)
			BottomBox.pack_start(BM, False, False, 0)
			BottomBox.pack_start(BR2, False, False, 0)
			BottomBox.pack_start(BR, False, False, 0)
			MainBox.pack_start(TopBox, False, False, 0)
			MainBox.pack_start(MiddleBox, False, False, 0)
			MainBox.pack_start(BottomBox, False, False, 0)
			return MainBox
		elif Type == 1:
			MainBox = gtk.Box(orientation=gtk.Orientation.VERTICAL, spacing=0)
			TopBox = gtk.Box(orientation=gtk.Orientation.HORIZONTAL, spacing=0)
			MiddleBox = gtk.Box(orientation=gtk.Orientation.HORIZONTAL, spacing=0)
			BottomBox = gtk.Box(orientation=gtk.Orientation.HORIZONTAL, spacing=0)
			UL = gtk.Image()
	...
	def gradient_box(self, width, height, color1, color2, power):
		...
		if power:
			cm = color1
		darea1.connect("draw", self.expose_gradient, 1, color1, color2, cm, width)
		darea2 = gtk.DrawingArea()
		if power:
			cm = color2
		darea2.connect("draw", self.expose_gradient, 2, color1, color2, cm, width)
	...
	def expose_gradient(self, widget, cr, zone, color1, color2, cm, width):
		if zone == 1:
			start = color1
			stop = cm
		if zone == 2:
	...
	def Create_zones(self):
		...
		if self.Advanced_Mode == False:
			self.AlienFX_Preview_Hbox = gtk.Box(orientation=gtk.Orientation.HORIZONTAL)
			self.AlienFX_Preview_Hbox.set_spacing(20)
			for zone in self.computer.regions.keys():
				self.AlienFX_Preview_Hbox.pack_start(self.Widget_Zone(self.computer.regions[zone]), False, False, 0)
			self.AlienFX_Preview_Eventbox.add(self.AlienFX_Preview_Hbox)
			bg_rgba = Gdk.RGBA()
			bg_rgba.parse(self.background_color)
			self.AlienFX_Preview_Eventbox.override_background_color(gtk.StateFlags.NORMAL, bg_rgba)
			self.AlienFX_Main_Windows.show_all()

	def Widget_Zone(self, zone, confId=0, line=False):
		"""That function creates a gtk object.
		This object is a not advanced zone box"""
		Zone_VBox = gtk.Box(orientation=gtk.Orientation.VERTICAL)
		set_type = 1
		Label = None
		height = 40
		...
		c1 = Gdk.color_parse('#' + self.configuration.area[zone.name][confId].color1)
		if zone.power_button:
			c2 = Gdk.color_parse('#' + self.configuration.area[zone.name][confId].color2)
		elif self.configuration.area[zone.name][confId].mode != "morph":
			c2 = Gdk.color_parse('#' + self.configuration.area[zone.name][confId].color1)
		else:
			c2 = Gdk.color_parse('#' + self.configuration.area[zone.name][confId].color2)
		...
		mode = gtk.Box(orientation=gtk.Orientation.VERTICAL)
		if not zone.power_button:
			...
			if zone.canLight:
				...
				mode.pack_start(EventFixed, False, False, 0)
			if zone.canBlink:
				blink = gtk.Image()
				...
				mode.pack_start(EventBlink, False, False, 0)
			if zone.canMorph:
				morph = gtk.Image()
				...
				mode.pack_start(EventMorph, False, False, 0)
		Color_Hbox = gtk.Box(orientation=gtk.Orientation.HORIZONTAL)
		Color_Hbox.pack_start(color1, False, False, 0)
		Color_Hbox.pack_start(color2, False, False, 0)
		Color_Hbox.pack_start(mode, False, False, 0)
		Table = self.Create_Border(set_type, Color_Hbox, Label, zone, confId)
		Zone_VBox.pack_start(Table, False, False, 0)
		return Zone_VBox
	...
	def Create_Line(self):
		...
		try:
			...
			pass
		l = 1
		self.AlienFX_Configurator_Table = gtk.Grid()
		self.AlienFX_Configurator_Table.set_row_spacing(20)
		self.AlienFX_Configurator_Table.set_column_spacing(20)
		old = 0
	...
	def Widget_Line(self, zone, l):
		...
		fg_rgba = Gdk.RGBA()
		fg_rgba.parse(self.text_color)
		title = gtk.Label(label=zone.description)
		title.override_color(gtk.StateFlags.NORMAL, fg_rgba)
		self.AlienFX_Configurator_Table.attach(title, 0, l - 1, 1, 1)
		for conf in range(len(self.configuration.area[zone.name])):
			confBox = self.Widget_Zone(zone, conf, line=True)
			self.AlienFX_Configurator_Table.attach(confBox, int(conf) + 1, l - 1, 1, 1)
		if not zone.power_button:
			AddConf = gtk.Button()
			AddConf.set_label("Add")
			AddConf.connect("clicked", self.on_Line_AddConf_pressed, zone, conf)
			self.AlienFX_Configurator_Table.attach(AddConf, int(conf) + 2, l - 1, 1, 1)
	...
	def AlienFX_Color_Panel(self):
		...
		self.AlienFX_Color_Panel_VBox = gtk.Box(orientation=gtk.Orientation.VERTICAL, spacing=5)
		...
		for c in default_color:
			if c == "select":
				...
				self.AlienFX_Color_Panel_VBox.pack_start(color_select_button, True, True, 0)
			else:
				fixed = gtk.Fixed()
				...
				rgba = Gdk.RGBA()
				rgba.parse('#' + c)
				color_EventBox.override_background_color(gtk.StateFlags.NORMAL, rgba)
				...
				if n > 3:
					HBox2.pack_start(box, False, False, 0)
				else:
					HBox1.pack_start(box, False, False, 0)
				n += 1

		self.AlienFX_Color_Panel_VBox.pack_start(HBox2, False, False, 0)
		self.AlienFX_Color_Panel_VBox.pack_start(HBox1, False, False, 0)
	...
	def on_Advanced_Button_Clicked(self, widget):
		if self.Advanced_Mode:
			...
			self.AlienFX_Inside_vBox.get_children()[2].hide()
			self.AlienFX_Inside_vBox.get_children()[2].set_size_request(-1, 0)
			self.AlienFX_Inside_vBox.get_children()[1].show()
			...
			self.AlienFX_Inside_vBox.get_children()[2].show()
			self.AlienFX_Inside_vBox.get_children()[2].set_size_request(-1, 350)
			self.AlienFX_Inside_vBox.get_children()[1].hide()
	...
	def on_Configuration_New(self, widget):
		if os.path.isfile(self.actual_conf_file):
			if not self.configuration.Check(self.actual_conf_file):
				messagedialog = gtk.Dialog(
					title="The configuration has changed ! Do you want to save it before creating a new one?",
					transient_for=None,
					flags=gtk.DialogFlags.MODAL | gtk.DialogFlags.DESTROY_WITH_PARENT)
				messagedialog.add_button("No", gtk.ResponseType.NO)
				messagedialog.add_button("Save", gtk.ResponseType.OK)
	...
	def __textbackground(self, widget, cr, image):
...
class Daemon_Controller:
	...
	def getResults(self):
		data = self.sock.recv(self.BUFSIZE).decode('utf-8')
		return data
