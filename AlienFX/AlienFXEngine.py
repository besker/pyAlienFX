...
class AlienFX_Driver(AllComputers):
    def __init__(self):
        ...
        if not self.FindDevice():
            raise RuntimeError(
                "No AlienFX USB controler found ! Go see the list of supported computer on : https://code.google.com/p/pyalienfx/wiki/SupportedComputer "
            )
        self.Take_over()
        # dev.claimInterface()
    ...
    def FindDevice(self):
        ...
        for computer in self.computerList.keys():
            dev = usb.core.find(
                idVendor=self.computerList[computer].vendorId,
                idProduct=self.computerList[computer].productId,
            )
            if dev is not None:
                print(
                    "Comnputer %s found ! Loading the parameters ..."
                    % self.computerList[computer].name
                )
...
class AlienFX_Controller:
    ...
    def Set_Color_Morph(
        self, Area, Color1, Color2, Save=False, Apply=False, block=0x01
    ):
        self.WaitForOk()
        request = AlienFX_Constructor(self.driver, Save, block)
        if type(Area) != list:
...
class AlienFX_Constructor(list):
    ...
    def Set_Blink_Color(self, Area, Color):
        self.Save()
        cmd = copy(self.void)
        legend = "Set Blink Color, Area : %s, Color : r = %s, g = %s, b = %s" % (
            hex(Area[0] * 65536 + Area[1] * 256 + Area[2]),
            hex((Color[0] // 16)),
            hex(Color[0] - (Color[0] // 16) * 16),
            hex(Color[1] // 16),
        )
        cmd[0] = self.computer.START_BYTE
    ...
    def Set_Morph_Color(self, Area, Color1, Color2):
        ...
        print("Color2 ==== >>> ", Color2)
        legend = (
            "Set Morph Color, Area : %s , Color1 : r = %s, g = %s, b = %s, Color2 : r = %s, g = %s, b = %s"
            % (
                hex(Area[0] * 65536 + Area[1] * 256 + Area[2]),
                hex((Color1[0] // 16)),
                hex(Color1[0] - (Color1[0] // 16) * 16),
                hex(Color1[1] // 16),
                hex(Color2[0] // 16),
                hex(Color2[0] - (Color2[0] // 16) * 16),
                hex(Color2[1] // 16),
            )
        )
    ...
    def Area(self, areas):  # gotta check the power button to understand it ...
        ...
        ret[1] = (
            area // 256 - ret[0] * 256
        )  # Takes the four first digit and remove the two first digit ret[0] = 0x12 => ret[0] * 256 = 0x1200
        ret[2] = (
            area - ret[0] * 65536 - ret[1] * 256
        )  # Same but remove the first 4 digit
        return ret

    def Set_Color(self, Area, Color, Id=0x01):
        self.Save()
        cmd = copy(self.void)
        legend = "Set Fixed Color, Area : %s, Color : r = %s, g = %s, b = %s" % (
            hex(Area[0] * 65536 + Area[1] * 256 + Area[2]),
            hex((Color[0] // 16)),
            hex(Color[0] - (Color[0] - (Color[0] // 16) * 16)),
            hex(Color[1] + 16),
        )
        cmd[0] = self.computer.START_BYTE
    ...
    def Reset(self, command):
        if command in [
            self.computer.RESET_ALL_LIGHTS_ON,
